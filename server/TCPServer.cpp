...
void grab_command(std::string request_to_base , SOCKET sock_of_current_user) {
	...
	for (int i = 0; i <= request_to_base.size(); i++) {
		command += request_to_base[i];
		if (request_to_base[i] != '(' || request_to_base[i] != '=' || request_to_base[i] != ';') {

			if (int(request_to_base[i]) == 32) {
				spaces += request_to_base[i];
			}

			if (command == spaces + "create" && command_executed != true) {
				std::cout << "create ::" << std::endl;
				command_executed = true;
				spaces.clear();
				method_CREATE( request_to_base , sock_of_current_user);
			}
			if (command == spaces + "set" && command_executed != true) {
	...
	/*
	int position_of_command_start = command.find("(");
	int position_of_command_end = command.find(")");
	std::string name_of_var = command.substr(position_of_command_start+1 , (position_of_command_end-position_of_command_start)-1);
	std::cout << name_of_var << std::endl;
	std::ofstream outfile(name_of_var+".txt");
...
void run_analyzer(char request[]  , SOCKET sock_of_current_user) {
	std::cout << "Analyzer is runned" << std::endl;
	std::string request_to_base;
	...
	for (int i = 0; i < 512; i++) {
		if (request[i] == ';') {
			...
			request_to_base += request[i];
		}
	}
	std::cout <<"Real:" << request_to_base << std::endl;
	grab_command(request_to_base , sock_of_current_user);
	
	request_to_base.clear();
		...
		while (true) {
			...
			for (int i = 0; i < socketCount; i++) {				//Server can only accept connection & receive msg from client. 
				...
				if (sock == listeningSocket) {				//Case 1: accept new connection.
					...
					std::string welcomeMsg = "Welcome to Rudik Base.\n";			//Notify client that he entered the chat. 
					send(client, welcomeMsg.c_str(), welcomeMsg.size() + 1, 0);
					std::cout << "New user joined the data base." << std::endl;			//Log connection on server side. 
